...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/create_handoff.py" implementing-user-auth
```

**Continuation handoff (linked to previous):**
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/create_handoff.py" "auth-part-2" --continues-from 2024-01-15-auth.md
```

**Validate before finalizing:**
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/validate_handoff.py" .claude/handoffs/2024-01-15-143022-implementing-auth.md
```
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/list_handoffs.py"
```

**Check if handoff is current:**
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/check_staleness.py" .claude/handoffs/2024-01-15-143022-implementing-auth.md
```
...
|--------|---------|-------|
| `create_handoff.py` | Generate new handoff with smart scaffolding | `python3 "${CLAUDE_SKILL_DIR}/scripts/create_handoff.py" [slug] [--continues-from <file>]` |
| `list_handoffs.py` | List available handoffs in a project | `python3 "${CLAUDE_SKILL_DIR}/scripts/list_handoffs.py" [path]` |
| `validate_handoff.py` | Check completeness, quality, and security | `python3 "${CLAUDE_SKILL_DIR}/scripts/validate_handoff.py" <file>` |
| `check_staleness.py` | Assess if handoff context is still current | `python3 "${CLAUDE_SKILL_DIR}/scripts/check_staleness.py" <file>` |
