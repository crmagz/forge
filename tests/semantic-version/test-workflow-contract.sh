...
for workflow in build-python.yml build-node.yml build-java.yml; do
  ...
  assert_contains "$workflow exposes a registry-neutral image tag" "$file" "      image-tag: \${{ steps.semver.outputs.ecr-tag }}"
  assert_contains "$workflow preserves the ECR tag output for compatibility" "$file" "      ecr-tag: \${{ steps.semver.outputs.ecr-tag }}"
  ...
  assert_contains "$workflow uses the registry-neutral image tag" "$file" "tag: \${{ needs.version.outputs.image-tag }}"
...
assert_registry_validation "GHCR rejects a registry-prefixed repository" 1 "ghcr" "" "ghcr.io/owner/image" "$node_registry_validator"
